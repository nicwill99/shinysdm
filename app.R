...
species_labels <- c(
  ...
  Phaneroptera_falcata     = "Gemeine Sichelschrecke",
...
#3.1 Model Validation mit cross validierung

evaluate_model_cv <- function(model, pred_vars, kfold = 5, seed = 1) {
  train_df <- model$model                 
  obs      <- train_df$presence
  set.seed(seed)  
  ks <- dismo::kfold(train_df, k = kfold, by = obs)
  cross_val_preds <- numeric(length = nrow(train_df))
  
  for (i in seq_len(kfold)) {
    
    cv_train <- train_df[ks != i, ]
    
    cv_test  <- train_df[ks == i, ]
    
    modtmp <- update(model, data = cv_train)
    
    cross_val_preds[ks == i] <- predict(
      
      modtmp, newdata = cv_test[, pred_vars, drop = FALSE], type = "response"
      
    )
    
  }
  thresh.method <- "MaxSens+Spec"
  thresh.dat <- data.frame(ID   = seq_len(length(obs)),
                           obs  = obs,
                           pred = cross_val_preds)
  
  
  thresh <- PresenceAbsence::optimal.thresholds(DATA = thresh.dat, req.sens = 0.85, req.spec = 0.85, FPC = 1, FNC = 1)
  
  best_thresh <- thresh[thresh$Method == thresh.method, 2]
  
  
  
  cmx.opt <- PresenceAbsence::cmx(DATA = thresh.dat, threshold = best_thresh)
  ...
  list(
    ...
    tss       = PresenceAbsence::sensitivity(cmx.opt, st.dev = FALSE) +
      
      PresenceAbsence::specificity(cmx.opt, st.dev = FALSE) - 1,
...
species_metrics <- setNames(
  
  lapply(species_models, evaluate_model_cv, pred_vars = c("bio1_mean", "bio12_mean")),
    ...
    mainPanel(width = 10,
              tabsetPanel(
                tabPanel("Hintergrund",
                         ...
                         p("Diese Anwendung befindet sich noch im Aufbau, daher kann es zu Bugs, längeren Ladezeiten oder kleineren Schönheitsfehlern kommen. Wenn euch etwas auffällt oder stört, gebt uns gerne Feedback, damit wir es in Zukunft besser machen können! :)"),
                         
                         h4("Was zeigen mir die Karten?"),
                         tags$ul(
                           tags$li("Karte 1: Vorhersage unter aktuellem Klima, mit beobachteten Vorkommen (weiße Punkte) zum Abgleich."),
                         ...
                         h4("Herkunft der Verbreitungsdaten"),
                         tags$ul(
                           tags$li("Nucifraga caryocatactes: European Nutcracker (Tannenhaeher), occurrence data from EBCC (European
Bird Census Council) atlas, access via GBIF (gbif.org)"),
                           tags$li("Merops apiaster: European Bee-Eater (Bienenfresser), occurrence data from EBCC (European Bird
Census Council) atlas, access via GBIF.org (11 December 2023) GBIF Occurrence Download https:
//doi.org/10.15468/dl.yy62zt"),
                           tags$li("Grus grus: Common Crane (Kranich), occurrence data from EBCC (European Bird Census Council)
                                   atlas, access via GBIF.org (11 December 2023) GBIF Occurrence Download https://doi.org/10.15468/
                                     dl.b9vcpb"),
                           tags$li("Coenagrion mercuriale: Southern Damselfly (Helm-Azurjungfer), occurrence data from IUCN"),
                           tags$li("Ophiogomphus cecilia: Green Snaketail (Gruene Flussjungfer), occurrence data from IUCN"),
                           tags$li("Phaneroptera falcata: Sickle-Bearing Bush-Cicket (Gemeine Sichelschrecke), occurrence data from Gbif"),
                           tags$li("Pinus cembra: Swiss Stone Pine (Zirbelkiefer), main food source of the nutcracker"),
                           tags$li("Corylus avellana: Common Hazel (Haselnuss), second food source of the nutcracker")
                         ),
                         h4("")
                ...
                tabPanel("Karten & Kurven",
                         ...
                         fluidRow(
                           ...
                           column(
                             width = 6,
                             h4("Veränderung (Referenz zu Klimaszenario)"),
                             withSpinner(
                         ...
                         helpText("Weiße Punkte auf der 1. Karte = tatsächlich beobachtete Vorkommen. ",
                                  "Lila in der 3. Karte = Zunahme, Orange = Abnahme der Vorkommenswahrscheinlichkeit."),
                         hr(),
...
server <- function(input, output, session) {
  ...
  make_map <- function(df, title, show_points = FALSE) {
    ...
    if (show_points) {
      p <- p + geom_point(data = presence_points(), aes(x = lon, y = lat),
                          color = "white", size = 0.7, alpha = 0.6)
    }
  ...
  output$model_info <- renderText({
    ...
    metrics <- species_metrics[[input$species]]
